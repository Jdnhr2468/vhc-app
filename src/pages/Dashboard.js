...
ListItemIcon, ListItemText, Chip, Button,
Drawer, Divider
} from '@mui/material';
import {
...
// ✅ Alerts Modal — открывается поверх страницы на мобильном
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              ...
              {!alert.read && (
                <Button
                  ...
                  sx={{
                    ml: 1, flexShrink: 0, fontSize: '0.7rem',
                    textTransform: 'none', borderRadius: '10px',
                    color: theme.primary, bgcolor: theme.primaryBg,
...
p: { xs: 1.5, md: 2.5 }, borderRadius: '20px',
...
<Box sx={{ bgcolor: `${color}18`, p: 1, borderRadius: '10px', display: 'flex' }}>
...
// ✅ на мобильном chip чуть меньше
'& .MuiChip-label': { px: 0.8 }
}} />
</Box>
<Typography sx={{
fontSize: { xs: '0.68rem', md: '0.8rem' },
color: theme.textSub, fontWeight: 600, mb: 0.3
}}>
{title}
</Typography>
...
fontWeight: 800, color: theme.textMain, letterSpacing: '-0.5px'
}}>
{value}
</Typography>
<Typography sx={{ fontSize: { xs: '0.65rem', md: '0.75rem' }, color: theme.textMuted, fontWeight: 600 }}>
{unit}
</Typography>
</Box>
</Paper>
...
const [alertsOpen, setAlertsOpen] = useState(false); // ✅ модальное окно алертов
...
```
  {/* ✅ MobileHeader — передаём onAlertClick чтобы открывать модал */}
  <MobileHeader
    title="Dashboard"
  ...
  <Box sx={{ flexGrow: 1, ml: { md: '250px' }, p: { xs: 2, md: 4 }, pb: { xs: 10, md: 4 }, width: '100%' }}>
        ...
        <Box sx={{ display: { xs: 'none', lg: 'flex' }, alignItems: 'center', gap: 1.5,
          ...
          {connectedDevices.map(d => (
            <Box key={d.name} sx={{ display: 'flex', alignItems: 'center', gap: 0.6 }}>
              <Box sx={{ width: 7, height: 7, borderRadius: '50%', bgcolor: theme.success }} />
              <Typography sx={{ fontSize: '0.72rem', fontWeight: 600, color: theme.textSub }}>{d.name}</Typography>
            </Box>
          ))}
        </Box>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, bgcolor: theme.successBg,
    ...
    {/* Приветствие мобильное */}
    <Box sx={{ display: { xs: 'block', md: 'none' }, mb: 2 }}>
      <Typography sx={{ fontSize: '1.1rem', fontWeight: 800, color: theme.textMain }}>
        Welcome back, {user?.displayName || user?.email?.split('@')[0]} 👋
      </Typography>
    ...
    {/* ✅ Биомаркеры — нормальная сетка на мобильном */}
    ...
    {/* Today's Insight */}
    <Paper elevation={0} sx={{
      p: { xs: 2, md: 2.5 }, borderRadius: '20px', mb: 2,
      ...
      <Button onClick={() => navigate('/reports')} sx={{
    ...
    {/* Weekly Activity — скрываем на мобильном (не нужно) */}
    <Box sx={{ display: { xs: 'none', md: 'block' } }}>
      <Paper elevation={0} sx={{
        ...
        <Typography sx={{ fontWeight: 700, color: theme.textMain, mb: 0.5 }}>Weekly Activity</Typography>
        <Typography sx={{ fontSize: '0.8rem', color: theme.textSub, mb: 2 }}>Steps over the past week</Typography>
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={history} barSize={18}>
            <CartesianGrid strokeDasharray="3 3" stroke="#F1F5F9" vertical={false} />
            <XAxis dataKey="day" tick={{ fontSize: 10, fill: theme.textMuted }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fontSize: 10, fill: theme.textMuted }} axisLine={false} tickLine={false} width={35} />
            <Tooltip
